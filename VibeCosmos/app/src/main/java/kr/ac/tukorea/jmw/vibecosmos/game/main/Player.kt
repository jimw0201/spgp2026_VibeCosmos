...
class Player(gctx: GameContext): AnimSprite(gctx, R.mipmap.player_run, 60f, frameCount = 32) {
    enum class State {
        RUN, UP_ATK, DOWN_ATK,
    }
    ...
    var state = State.RUN
        set(value) {
            val (resId, frameCount) = when (value) {
                ...
                State.UP_ATK -> R.mipmap.up_atk to 30
                State.DOWN_ATK -> R.mipmap.down_atk to 22
            }
